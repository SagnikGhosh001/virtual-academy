...
public class MarksServiceImpl implements MarksService {
	...
	public void updateMarkbyId(int id, MarksDTO marks) {
		Marks existMarks = marksDao.findById(id)
		...
		Sub sub = subDao.findById(marks.getSubId()).orElseThrow(() -> new ResourceNotFoundException("sub","id",marks.getSubId()));
		Sem sem = semDao.findById(existMarks.getSub().getSem().getId()).orElseThrow(() -> new ResourceNotFoundException("sem","id",existMarks.getSub().getSem().getId()));
	...
	public void delteMarksById(int id,MarksDTO marksDTO) {
		...
		if (teacher.getDept().equals(existMarks.getReg().getDept())  && teacher.getSem().contains(existMarks.getSub().getSem())) {
